...
- scheduled 성공 응답은 actor 내부에서 최소 180초 재사용하고, 재사용 값은 원래 관측 시각을 유지한 `recent` freshness로 반환한다. 사용자 수동 새로고침은 성공 캐시만 우회한다. `429`는 수동 요청에도 `Retry-After` 숫자 또는 기본 15분 backoff를 적용해 usage endpoint 과호출을 방지한다.
...
- scheduled 성공 결과는 5분 재사용하며 재사용 값은 원래 관측 시각을 유지한 `recent` freshness로 반환한다. 사용자 수동 새로고침은 성공 캐시를 우회한다. CLI 없음·로그인 실패·trusted workspace 없음·output drift는 typed state이며 정적 요금제 상한으로 현재 사용률을 추정하지 않는다.
...
- scheduled 성공 결과는 5분 재사용하며 재사용 값은 원래 관측 시각을 유지한 `recent` freshness로 반환하고, 사용자 수동 새로고침은 성공 캐시를 우회한다. process timeout은 25초다. 실행 내부 retry·loop·모델 호출·login/logout·credential write-back은 없다.
