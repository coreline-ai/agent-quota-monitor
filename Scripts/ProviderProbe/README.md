...
- `gemini_cli_quota_probe.py`: 공식 Antigravity CLI의 `/usage`만 실행해 Gemini 주간·5시간 field 존재 여부와 설정 불변만 출력
...
- `test_gemini_cli_quota_probe.py`: Gemini group 격리·ANSI 제거·실행 파일·workspace 검증 자체 테스트
...
python3 Scripts/ProviderProbe/gemini_cli_quota_probe.py --live
...
- Gemini는 공식 `agy /usage`의 `GEMINI MODELS` 그룹만 읽고 Claude/GPT 그룹·계정 정보·실제 percentage·raw TUI를 출력하거나 저장하지 않는다. 자동 업데이트도 probe 동안 비활성화하며 terminal protocol 응답이 단계 timeout을 재시작하지 않게 한다.
